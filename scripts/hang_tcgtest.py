#!/usr/bin/env python3
"""Run the hanging compile under bare qemu PLUS extra qemu args (e.g. -d nochain)
to test whether a TCG-translation change masks the hang."""
import re, subprocess, sys, time, serial, pathlib

REPO = pathlib.Path(__file__).resolve().parent.parent
KERNEL = REPO / "zig-out/bin/yasos_kernel"
FATIMG = REPO / "scripts/fatimg/fatimg"
FATDISK_OFFSET = 0x80EC0000 - 0x80000000
PTY_RE = re.compile(r"char device redirected to (\S+)")

backing, srcfile, cmd = sys.argv[1], sys.argv[2], sys.argv[3]
extra = sys.argv[4].split() if len(sys.argv) > 4 else []
wait = float(sys.argv[5]) if len(sys.argv) > 5 else 30

img = "/tmp/tcg_fd.img"
subprocess.run([str(FATIMG), "mkfs", img, "1024"], check=True)
subprocess.run([str(FATIMG), "cp", img, srcfile, "IN.C"], check=True)
data = pathlib.Path(img).read_bytes()
subprocess.run(["truncate", "-s", "16M", backing], check=True)
with open(backing, "r+b") as f:
    f.seek(FATDISK_OFFSET); f.write(data)

logf = open("/tmp/tcg_qemu.log", "w+")
qcmd = ["qemu-system-arm", "-machine", "mps2-an505,memory-backend=mem0",
        "-object", f"memory-backend-file,id=mem0,size=16M,mem-path={backing},share=on",
        "-cpu", "cortex-m33", "-display", "none", "-monitor", "none",
        "-semihosting-config", "enable=on,target=native",
        "-serial", "pty", "-kernel", str(KERNEL)] + extra
print("QEMU:", " ".join(qcmd[-len(extra):]) if extra else "(bare)")
proc = subprocess.Popen(qcmd, stdin=subprocess.DEVNULL, stdout=logf, stderr=subprocess.STDOUT)
pty = None; deadline = time.time() + 20
while time.time() < deadline:
    m = PTY_RE.search(pathlib.Path("/tmp/tcg_qemu.log").read_text(errors="ignore"))
    if m: pty = m.group(1); break
    time.sleep(0.05)
if not pty:
    print("no pty"); print(pathlib.Path("/tmp/tcg_qemu.log").read_text()[:2000]); sys.exit(1)
time.sleep(7)
ser = serial.Serial(pty, 115200, timeout=0.2)
ser.write((cmd + "\n").encode()); ser.flush()
end = time.time() + wait; out = b""
while time.time() < end:
    n = ser.in_waiting
    if n:
        c = ser.read(n); out += c
        sys.stdout.write(c.decode(errors="replace")); sys.stdout.flush()
    else: time.sleep(0.1)
print("\n>> done")
try:
    ps = subprocess.run(["ps","-eo","pid,comm"], capture_output=True, text=True).stdout
    for line in ps.splitlines():
        if line.strip().endswith("qemu-system-arm"):
            subprocess.run(["kill","-9", line.split()[0]])
except Exception: pass
