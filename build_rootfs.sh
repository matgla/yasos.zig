...
build_cross_compiler()
{
  ...
  cd tinycc
  mkdir -p bin
  ./configure --extra-cflags="-DTCC_DEBUG=2 -g -O0 -DTARGETOS_YasOS=1 -Wall -Werror" --enable-cross --config-asm=yes --config-bcheck=no --config-pie=yes --config-pic=yes --prefix="$PREFIX" --sysroot="$SCRIPT_DIR/rootfs"
  if [ $? -ne 0 ]; then
    exit -1;
  fi
