...
pub fn Uart(comptime index: usize, comptime pins: interface.uart.Pins) type {
    _ = pins;
    // MPS3-AN524 CMSDK UARTs live in apb_ppcexp2 at 0x41303000 (uart0), +0x1000
    // per index (verified against `info mtree` + a bare-metal probe). Same CMSDK
    // register layout as the an505 — only the base moves.
    const base: usize = switch (index) {
        ...
        3 => 0x41306000,
        4 => 0x41307000,
        else => @compileError("MPS3-AN524 exposes UART0..UART5; 0..4 wired here"),
    };
