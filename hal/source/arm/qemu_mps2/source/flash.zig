...
// Memory-mapped "flash" for QEMU. On the MPS2-AN505 the romfs image is embedded
// into the kernel ELF (see startup/rootfs.S) and loaded into RAM by QEMU's
// `-kernel` loader; the board points this at the blob's base address so the
// kernel's RomFs mount path is identical to the real board.
