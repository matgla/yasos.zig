...
// Generic 32-bit MMIO register helper for plain Cortex-M cores (no RPXXXX
// SET/CLEAR/XOR atomic aliases). Only read/write/update are used by the shared
// armv8-m register definitions.
pub fn Mmio(comptime RegisterFieldDescription: type) type {
    const size = @bitSizeOf(RegisterFieldDescription);
    if (size != 32) {
        @compileError("Register must be 32-bit");
    }
