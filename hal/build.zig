...
fn replace_tokens(b: *std.Build, linker_script: []const u8) []u8 {
    ...
    while (token_position) |start_position| {
        const hal_root = b.root.toString(b.graph.arena) catch @panic("OOM");
...
pub const Builder = struct {
    pub fn configureBoard(_: *const Builder, b: *std.Build, board: []const u8, execName: []const u8, root_file: std.Build.LazyPath, optimize: std.builtin.OptimizeMode, config_file: []const u8) !*std.Build.Module {
        ...
        if (mcu.module("hal").resolved_target) |target| {
            ...
            const cwd_path = std.Io.Dir.cwd().realPathFileAlloc(b.graph.io, ".", b.allocator) catch |err| {
                return err;
            };
            const hal_path = b.pathJoin(&.{ cwd_path, "hal" });
            const config_zig_relative = std.fs.path.relative(b.allocator, cwd_path, &b.graph.environ_map, hal_path, config_zig_ext) catch |err| {
                return err;
            };
