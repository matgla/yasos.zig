...
pub const RomFs = interface.DeriveFromBase(ReadOnlyFileSystem, struct {
    ...
    pub fn init(allocator: std.mem.Allocator, device_file: IFile, start_offset: u64) !RomFs {
        const fs = try FileSystemHeader.init(allocator, device_file, @intCast(start_offset));
        return RomFs.init(.{
    ...
    pub fn supports_symlinks(self: *const Self) bool {
        _ = self;
        // The image format has a SymbolicLink file type, and the rootfs uses
        // it: build_rootfs.sh installs toybox with --symlink.
        return true;
    }
