...
/// Whether a `[]const u8` looks like something that can actually be read. Crude,
/// and not a memory-map lookup: this runs from the fault handler, where the map
/// may itself be what is broken. It only has to catch the shapes a dangling
/// slice takes -- an all-ones or null pointer, an unmapped high address, or a
/// length no log line could have.
fn readable_string(s: []const u8) bool {
    ...
    // window is the PPB and the EXC_RETURN encodings. Gated on pointer width
    // because ordinary host addresses sit far above that line.
...
fn emitValue(out: *Out, v: Value, spec: Spec) void {
    ...
    switch (v) {
        ...
        // A `{s}` whose slice is dangling must not take the kernel down: the
        // HardFault handler formats its own postmortem through here, so a fault
        // inside it dies a second time and loses the original frame. Print the
        // offending pointer and length instead.
