...
// ============================================================================
// C99 classification helpers (targets of the fpclassify/isinf/... macros)
// ============================================================================
...
int __fpclassifyd(double x) {
  ...
  unsigned long long mantissa = b.bits & 0x000fffffffffffffULL;
  if (exponent == 0x7ffu)
...
int __fpclassifyf(float x) {
  math_float_bits b;
  b.value = x;
  unsigned int exponent = (b.bits >> 23) & 0xffu;
  unsigned int mantissa = b.bits & 0x007fffffu;
  if (exponent == 0xffu)
    return mantissa ? FP_NAN : FP_INFINITE;
  if (exponent == 0)
    return mantissa ? FP_SUBNORMAL : FP_ZERO;
...
int __signbitd(double x) {
  math_double_bits b;
  b.value = x;
  return (int)(b.bits >> 63);
}

int __signbitf(float x) {
  math_float_bits b;
  b.value = x;
  return (int)(b.bits >> 31);
}
