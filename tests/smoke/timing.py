...
Measures compile, yasld load, and execute time on the target device
(excludes file transfer overhead). Results are printed as a pytest
terminal summary and persisted to a JSON file for further analysis.
"""
...
class CaseTiming:
    ...
    def target_total_ms(self):
        return self.compile_ms + self.loader_ms + self.execute_ms


timing_results: list[CaseTiming] = []

LOADER_TIMING_RE = re.compile(
    r"yasld-bench (?P<kind>executable|library) path=(?P<path>\S+) pid=(?P<pid>\d+) us=(?P<us>\d+)"
)
...
def attach_loader_timing(timing: CaseTiming, log_path: str, executable_path: str):
    """Attach the most recent executable-load timing for *executable_path* from *log_path*."""
        ...
        with open(log_path, "r", encoding="utf-8", errors="ignore") as fh:
            for line in fh:
                ...
                if match.group("path") != executable_path:
                    continue
                timing.loader_ms = int(match.group("us")) / 1000.0
    except OSError:
        return
...
def format_timing_report(terminalreporter):
    ...
    total_loader = sum(t.loader_ms for t in timing_results)
    total_execute = sum(t.execute_ms for t in timing_results)
    total = total_compile + total_loader + total_execute
    ...
    terminalreporter.write_line(
        ...
        f"loader: {total_loader / 1000:.2f}s, "
    ...
    terminalreporter.write_line(
        f"  {'#':<4} {'Test ID':<55} {'Compile':>10} {'Loader':>10} {'Execute':>10} {'Total':>10}"
    )
    terminalreporter.write_line(f"  {'-' * 100}")
    ...
    if len(categories) > 1:
        ...
        terminalreporter.write_line(
            ...
            f"{'Compile':>12} {'Loader':>12} {'Execute':>12} {'Total':>12} {'Avg':>10}"
        )
        terminalreporter.write_line(f"  {'-' * 84}")
        for cat_name in ("tests2", "ir_tests", "gcc_compile", "gcc_execute"):
            ...
            cat_total = cat_compile + cat_loader + cat_execute
