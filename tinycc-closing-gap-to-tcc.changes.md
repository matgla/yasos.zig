...
| G11 | Scorecard `-O0` figures: tcc -O0 112,880,275, optimizer 2.23x, -O0 vs gcc 2.55x, and doubles "the optimizer bought them nothing" (1.01x) | **The `-O0` arm was measuring optimized code.** The benchmark links a prebuilt `lib/fp/libsoftfp.a` that is always built at -O2, and every `double_*` kernel is a loop of `bl __aeabi_d*`, so ~99% of the doubles' -O0 cycles were -O2 code. Re-measured 2026-08-30 with the FP runtime rebuilt at -O0: tcc -O0 **119,507,836**, optimizer **2.37x**, -O0 vs gcc **2.70x**, doubles gain **1.80x** (they are 2.95x gcc at -O0 against 2.67x for the other 24 — further behind at both ends, not skipped). gcc -O2, tcc -O2 and the 1.14x headline are unchanged, reproduced to the cycle. | `tests/benchmarks/CMakeLists.txt:95,137,220-224`; `lib/fp/soft/Makefile:14`; board run 2026-08-30 |
| G12 | "The median function is exactly one point zero zero" quoted as what the typical function does (Scorecard VO; `s13_b4` card), and "4,358 hand-written tests" on that card | **The median is the same trap as the 0.93x aggregate.** 1.00x is the *whole-corpus* median and the five generated files make it — their own median is **0.81x** (tcc shorter on 8,111 of their 10,245). With them out the median is **1.10x**: tcc longer on 5,767 of the 10,552 hand-written functions, shorter on 1,912, level on 2,873. Never quote a median without the cut it came from. The test count was wrong too — 4,363 are *submitted*, **4,261** compile to at least one function (102 have none), so the hand-written half is **4,256**, not 4,363 − 5. | `regression_disasm.py --suite all -j24 --csv`, re-run in full 2026-08-30 at `803a2f25` |
